import { readExecutorConnectionStatus, startExecutorLogin } from "@/lib/provider-connections/executor-client";
import { getOwnedProviderConnection, markExecutorOffline, markProviderConnectionAction, updateProviderConnectionFromExecutor, withProviderConnectionLock } from "@/lib/provider-connections/store";
...
function hasExecutorErrorCode(error: unknown, code: string): error is { code: string; message: string } {
  ...
  try {
    ...
    const result = await withProviderConnectionLock(connectionId, async () => {
      ...
      try {
        ...
        if (hasExecutorErrorCode(error, "account_already_connected")) {
          const live = await readExecutorConnectionStatus(connection.sessionRef);
          await updateProviderConnectionFromExecutor(connection.id, live, { loginId: null });
          return {
