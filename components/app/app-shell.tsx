  ...
  return (
    <div className="flex min-h-screen flex-col bg-lab-bg text-lab-text">
      <header className="sticky top-0 z-40 flex h-14 shrink-0 items-center border-b border-lab-border bg-lab-surface-1 px-4">
        <Link href="/" className="lab-wordmark shrink-0">Lab<span>IA</span></Link>
        <nav aria-label="Navegação principal" className="ml-6 flex min-w-0 flex-1 items-center gap-1 overflow-x-auto">
          {primaryNav.map((item) => {
          ...
          {futureNav.map((label) => <span key={label} aria-disabled="true" className="flex h-8 shrink-0 items-center gap-2 rounded-control px-3 text-sm font-medium text-lab-text-muted opacity-70">{label}<Badge className="px-1.5 py-0 text-[10px]">em breve</Badge></span>)}
        </nav>
        <div className="ml-4 flex shrink-0 items-center gap-3">
          <Badge variant="cost">{monthCostLabel}</Badge>
          <div aria-label="Usuário" className="flex size-9 items-center justify-center rounded-control border border-lab-border bg-lab-surface-2 text-lab-text-dim"><CircleUserRound className="size-4" /></div>
        </div>
      </header>
