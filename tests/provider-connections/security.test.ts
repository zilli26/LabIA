...
describe("provider connection local access", () => {
  ...
  it("aceita x-forwarded-host local idêntico adicionado pelo Next e recusa divergências", () => {
    expect(() => assertLocalConnectionsRequest(localRequest({ "x-forwarded-host": "localhost:3000" }))).not.toThrow();
    expect(() => assertLocalConnectionsRequest(localRequest({ "x-forwarded-host": "127.0.0.1:3000" }))).toThrowError(/LabIA local/);
    expect(() => assertLocalConnectionsRequest(localRequest({ "x-forwarded-host": "evil.example" }))).toThrowError(/LabIA local/);
  });

  it("recusa bootstrap sem origem, com host remoto ou fetch-site cross-site", () => {
    const withoutOrigin = new Request("http://localhost:3000/api/provider-connections/session", {
      method: "POST",
      headers: { host: "localhost:3000", "sec-fetch-site": "same-origin" },
    });
    expect(() => assertLocalConnectionsRequest(withoutOrigin)).toThrowError(/origem|token/i);

    const remote = new Request("http://example.test/api/provider-connections/session", {
      method: "POST",
      headers: { host: "example.test", origin: "http://example.test", "sec-fetch-site": "same-origin" },
    });
    expect(() => assertLocalConnectionsRequest(remote)).toThrowError(/LabIA local/);

    expect(() => assertLocalConnectionsRequest(localRequest({ "sec-fetch-site": "cross-site" }))).toThrowError(/cross-origin/i);
  });
