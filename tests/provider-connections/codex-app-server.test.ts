...
describe("CodexAppServerClient", () => {
  ...
  it("exige o userAgent da versão homologada e permite override explícito", async () => {
    ...
    await expect(access(path.join(root, "123e4567-e89b-42d3-a456-426614174010"))).rejects.toThrow();
  });

  it("encerra o App Server e remove o CODEX_HOME em timeout do initialize", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "labia-codex-test-"));
    tempDirs.push(root);
    ...
    await expect(access(path.join(root, "123e4567-e89b-42d3-a456-426614174011"))).rejects.toThrow();
  });
  ...
  it("rejects a connecting status without a login id before reading a cached account", async () => {
    ...
    expect(status.authStatus).toBe("error");
    expect(status.errorCode).toBe("login_attempt_missing");
    expect(fake.requests.map((item) => item.method)).not.toContain("account/read");
    manager.closeAll();
  });
