...
Leia também `docs/LABIA-CHATGPT-SKILL.md` e `docs/MCP-CONTRATO-PILOTO.md`. Use a skill para transformar pedidos de vídeo em receita e Flow draft; use o contrato para distinguir tools existentes de tools apenas propostas. Não declare que existe MCP remoto até que autenticação, workspace e servidor estejam comprovados.
...
Atualização da publicação: o projeto completo foi enviado no commit e3be150 e seu deploy automático na Vercel ficou READY; os nós de vídeo estão publicados. A verificação encontrou o Supabase LabIA INACTIVE e a Biblioteca com erro 500 de conexão ao banco. Antes de qualquer teste com dados/geração, tratar essa pendência operacional e conferir banco/Storage/worker. A publicação não reativou o banco nem executou gerações. Consulte a revisão mais recente de RETOMADA para possíveis atualizações posteriores.
