...
type LoginAttempt = {
  ...
  startedAt: number;
  expiresAt: number;
  instruction: Omit<LoginInstruction, "expiresAt"> | null;
  rehydrated?: boolean;
...
export class CachedAccountConnectedError extends Error {
  readonly code = "account_already_connected";

  constructor() {
    super("A conta ChatGPT jÃ¡ estÃ¡ conectada. Desconecte a conta explicitamente antes de iniciar outro login.");
    this.name = "CachedAccountConnectedError";
  }
}

export type ExecutorImageResult = {
  requestId: string;
  asset: CodexImageAsset;
  raw: CodexImageItem;
};
...
export class ProviderExecutorManager {
  ...
  async startLogin(sessionRef: string, method: OpenAiLoginMethod) {
    ...
    if (cached.connected) throw new CachedAccountConnectedError();
  ...
  async status(sessionRef: string, expectedLoginId?: string | null, expectedLoginExpiresAt?: string | null): Promise<ExecutorAccountStatus> {
    ...
    try {
      ...
      if (!session.attempt && expectedLoginId && expectedLoginExpiresAt) {
        this.rehydrateAttempt(session, expectedLoginId, expectedLoginExpiresAt);
      }
      await this.expireAttemptIfNeeded(session);
      const snapshot = accountSnapshot(response);
      const attempt = session.attempt;

      if (snapshot.connected && (!attempt || attempt.rehydrated)) {
        session.attempt = null;
        return this.connectedStatus(snapshot);
      }
  ...
  private rehydrateAttempt(session: SessionState, loginId: string, expiresAtValue: string) {
    const expiresAt = Date.parse(expiresAtValue);
    if (!Number.isFinite(expiresAt) || session.attempt) return;
    session.attempt = {
      loginId,
      method: "chatgptDeviceCode",
      startedAt: Math.max(0, expiresAt - this.loginTtlMs),
      expiresAt,
      instruction: null,
      rehydrated: true,
    };
  ...
  private connectedStatus(snapshot: ReturnType<typeof accountSnapshot>) {
    return {
      authStatus: "connected",
      executorStatus: "online",
  ...
  async startImageGeneration(sessionRef: string, input: CodexImageStartInput) {
    const session = this.getOrCreateSession(sessionRef);
    await session.client.assertImageGenerationReady(input.model);
    const handle = await session.client.startImageGeneration(input);
    return { jobId: handle.id, model: handle.model };
  }
  ...
  async waitForImageGeneration(sessionRef: string, jobId: string): Promise<ExecutorImageResult> {
    const session = this.getOrCreateSession(sessionRef);
    const item = await session.client.waitForImageGeneration(jobId);
    return {
      requestId: item.id,
      asset: await session.client.normalizeImageResult(item),
      raw: item,
    };
  }
...
export function createProviderExecutorServer(options?: { token?: string; manager?: ProviderExecutorManager }) {
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      const match = url.pathname.match(/^\/connections\/([^/]+)\/(start|status|cancel|logout|image\/start|image\/result|image\/options)$/);
      ...
      if (action === "status" && req.method === "GET") {
        const expectedLoginId = url.searchParams.has("expectedLoginId")
          ...
          : undefined;
        const expectedLoginExpiresAt = url.searchParams.has("expectedLoginExpiresAt")
          ? (url.searchParams.get("expectedLoginExpiresAt") || null)
          : undefined;
        ...
        if (expectedLoginExpiresAt && expectedLoginExpiresAt.length > 64) {
          return sendJson(res, 400, { error: { code: "invalid_login_expiry", message: "Expiração de login inválida." } });
        }
        return sendJson(res, 200, await manager.status(sessionRef, expectedLoginId, expectedLoginExpiresAt));
      }
      if (action === "image/options" && req.method === "GET") {
        return sendJson(res, 200, await manager.readImageOptions(sessionRef));
      }
      ...
      if (action === "image/start") {
        ...
        if (typeof body.model !== "string" || typeof body.prompt !== "string") {
          return sendJson(res, 400, { error: { code: "invalid_image_input", message: "Modelo e prompt são obrigatórios." } });
        }
        const input: CodexImageStartInput = {
          model: body.model,
          prompt: body.prompt,
          referencedImagePaths: Array.isArray(body.referencedImagePaths) ? body.referencedImagePaths.filter((value): value is string => typeof value === "string") : undefined,
          numLastImagesToInclude: typeof body.numLastImagesToInclude === "number" ? body.numLastImagesToInclude : undefined,
        };
        return sendJson(res, 200, await manager.startImageGeneration(sessionRef, input));
      }
      if (action === "image/result") {
        const body = await readJson(req);
        if (typeof body.jobId !== "string" || body.jobId.length > 512) {
          return sendJson(res, 400, { error: { code: "invalid_image_job", message: "Job de imagem inválido." } });
        }
        return sendJson(res, 200, await manager.waitForImageGeneration(sessionRef, body.jobId));
      }
      ...
      if (error instanceof CachedAccountConnectedError) {
        return sendJson(res, 409, { error: { code: error.code, message: error.message } });
      }
