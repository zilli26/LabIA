...
export async function withProviderConnectionLock<T>(connectionId: string, work: () => Promise<T>) {
...
export async function markProviderConnectionAction(
  ...
  const row = await updateOwnedRow(connectionId, scope, {
    ...
    accountLabel: data.accountLabel === undefined ? owned.accountLabel : data.accountLabel,
    planType: data.planType === undefined ? owned.planType : data.planType,
