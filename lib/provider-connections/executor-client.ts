...
async function executorFetch<T>(pathname: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (!response.ok) throw new Error(payload.error?.message || `Executor respondeu HTTP ${response.status}`);
    return payload as T;
  } catch (error) {
...
export async function readExecutorConnectionStatus(sessionRef: string, expectedLoginId?: string | null) {
  const query = expectedLoginId === undefined
    ? ""
    : `?expectedLoginId=${expectedLoginId ? encodeURIComponent(expectedLoginId) : ""}`;
